...
simple_union_test() ->
	{ok, {calc, {calc,"radiohead"}, {calc,"queen"}, {oper,"+"}}} = OK = least_parse:string("radiohead + queen"),
	OK = least_parse:string("(radiohead + queen)"),
	OK = least_parse:string("(radiohead+queen)").

composed_names_union_test() ->
	{ok, {calc, {calc,"radio head"}, {calc,"pink floyd"},{oper,"+"}}} = OK = least_parse:string("radio head + pink floyd"),
